...
#    #ZKSYNC LITE (V1): Block Explorer
  ...
  Scenario: Check redirection for the "Send to" page (Withdraw)
    ...
    Then Element with "text" " Arriving in ~3+ hours " should be "visible"
    When I click by "text" with " Arriving in ~3+ hours " value
    Then Element with "text" " Arriving in ~3+ hours " should be "clickable"
  ...
  Scenario: Check redirection for the "Confirm transaction" modal (Withdraw)
    ...
    Then Modal card element with the "//*[text()=' Arriving in ~3+ hours ']" xpath should be "visible"
    When I click by the "//*[text()=' Arriving in ~3+ hours ']" text element on the Modal card
    Then New page has "https://era.zksync.io/docs/reference/troubleshooting/withdrawal-delay.html#withdrawal-delay" address
