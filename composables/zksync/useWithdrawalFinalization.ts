...
import type { FinalizeL1DepositParamsStruct } from "zksync-ethers/build/types";
...
export default (transactionInfo: ComputedRef<TransactionInfo>) => {
  ...
  const finalizeWithdrawalParams = ref<FinalizeL1DepositParamsStruct | undefined>();
  ...
  const getFinalizationParams = async () => {
    ...
    return await wallet.getFinalizeDepositParams(transactionInfo.value.transactionHash);
  };
